...
% Threats injected as signal-level modifications (Approach A).
% params.active_threat selects the threat; 'none' reproduces the A3 baseline.
%   Implemented (all 6): jamming, noise_burst, path_loss, antenna_fault, spoofing, reactive_jamming.
...
switch lower(p.active_threat)
    ...
    case 'noise_burst'
        % Bursty jamming: additive noise gated ON/OFF in time.
        jammerPower = 10^(p.jsr_db / 10);
    ...
    case 'path_loss'
        % Path Loss: constant attenuation of the signal (weaker Tx power at Rx).
    ...
    case 'antenna_fault'
        % Antenna Fault: severe attenuation gated ON/OFF in time (signal drop-out).
        faultLinear = 10^(-p.fault_atten_db / 20);
        onSymbols   = round(p.fault_duty * p.fault_period);
        chart_th.Script = sprintf([ ...
            'function y = fcn(u)\n' ...
            '%%#codegen\n' ...
            'persistent k\n' ...
            ...
            '    k = k + 1;\n' ...
            'end\n' ...
            'end\n'], faultLinear, p.fault_period, onSymbols);

    case 'spoofing'
        % Spoofing: inject a counterfeit QPSK signal mimicking a legitimate transmission.
    ...
    case 'reactive_jamming'
        % Reactive Jamming: smart jammer that transmits ONLY when it senses an active
        % signal (energy above threshold). Efficient and hard to detect — hides in silence,
        % strikes on transmission. Justifies the DQN (static policy can't beat an adaptive foe).
        jammerPower = 10^(p.jsr_db / 10);
        chart_th.Script = sprintf([ ...
            'function y = fcn(u)\n' ...
            '%%#codegen\n' ...
            ...
            '    end\n' ...
            'end\n' ...
            'end\n'], jammerPower, p.reactive_threshold);
