...
%              sinr, ber, rssi, plr, env_corr (as returned by extract_closed_loop_frames)
%   k          current frame index in M
%   tw         temporal window [frames], causal, clipped to the start of M
%   frame_dur  frame duration [s]
%   raw        1 x 8: [sinr, ber, rssi, plr, var_rssi, dber_dt, burst_ratio, env_corr]
%              missing values (e.g. BER of an incomplete frame) are set to 0
names = {'sinr', 'ber', 'rssi', 'plr', 'var_rssi', 'dber_dt', 'burst_ratio', 'env_corr'};
w0 = max(1, k - tw + 1);
...
raw = [M.sinr(k), M.ber(k), M.rssi(k), M.plr(k), var_rssi, dber, burst, M.env_corr(k)];
raw(~isfinite(raw)) = 0;
end
