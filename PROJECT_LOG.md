...
### Next
Re-run `run_closed_loop_diagnostic`, `eval_speed_robustness` and the KPI chain with the fixes; commit D29–D30; then improvement (4): episodic closed loop with dwell/hysteresis and recovery time in decision cycles (KPI #3 as written). README results section to be rewritten once these numbers are final.
