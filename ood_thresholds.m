...
%   from ood_scores.m (the production score since D44). Cached in
%   data/ood_thresholds.mat and recomputed when the detector is newer.
...
if isfile(cache) && dir(cache).datenum >= dir('data/trained_detector.mat').datenum
    C = load(cache, 'T');
    if isfield(C.T, 'retain') && C.T.retain == retain && isfield(C.T, 'maha'), T = C.T; return; end
end
...
T = struct('retain', retain, 'msp', lowq(msp, 1 - retain), 'energy', lowq(en, 1 - retain), ...
    'maha', lowq(mh, 1 - retain), 'n_val', numel(msp));
save(cache, 'T');
end
